...
class OpenAccountChart(models.TransientModel):
    ...
    def build_domain(self, wiz_id=None, account_ids=None, context=None):
        ...
        if account_ids:
            ...
            sub_accounts = (
                ...
                .search([("id", "child_of", account_ids)])
            )
            # sub_accounts = (
            #     self.env["account.account"]
            #     .with_context({"show_parent_account": True})
    ...
    def account_chart_open_window(self):
        ...
        ap_url = (
            ...
            "active_id=%s&amp;active_model=%s"
        ) % (self.id, "account.open.chart")
        result = {
        ...
        if (
            not self.env["account.account"].search(
                [
            ...
            and self.report_type == "account"
        ):
            result = self.env.ref("account.action_account_form").read([])[0]
        # else:
        # result = self.env.ref('account.action_account_form').read([])[0]
        # self.report_type = 'account_type'
        lines = self.with_context(**used_context).get_lines(wiz_id=self.id)
        if not lines:
    ...
    def format_amount_to_str(self, value, currency):
        ...
        formatted_amount = (
            lang.format(fmt, currency.round(value), grouping=True, monetary=True)
            .replace(r" ", "\N{NO-BREAK SPACE}")
            .replace(r"-", "-\N{ZERO WIDTH NO-BREAK SPACE}")
        )
        pre = post = ""
        if currency.position == "before":
            pre = "\N{NO-BREAK SPACE}".format(symbol=currency.symbol or "")
        else:
    ...
    def float_html_formatting(self, value, company):
        ...
        return (
            html_formatting and self._amount_to_str(value, company.currency_id) or value
        )
    ...
    def get_html(self, given_context=None):
        res = self.browse()
        if (
            given_context.get("active_id")
            and given_context.get("active_model") == "account.open.chart"
        ):
    ...
    def show_journal_items(self, wiz_id=None, line_id=None, acc_name=""):
        ...
        if report_type == "account_type" and line_id < 0:
            at_datas = list(
                filter(
                    lambda x: x["parent_id"] == line_id, report_obj.account_type_data()
                )
            )
            for at_data in at_datas:
