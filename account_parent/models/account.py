...
class AccountAccount(models.Model):
    ...
    def _compute_account_root(self):
        ...
        for record in self:
            if record.account_type != "view":
                record.root_id = (
                    record.code
                    and (ord(record.code[0]) * 1000 + ord(record.code[1:2] or "\x00"))
                    or False
                )
            else:
    ...
    def _compute_values(self):
        ...
        for account in self:
            sub_accounts = self.with_context(show_parent_accoun=True).search(
    ...
    balance = fields.Float(compute="_compute_values", digits=(16, 4))
    credit = fields.Float(compute="_compute_values", digits=(16, 4))
    debit = fields.Float(compute="_compute_values", digits=(16, 4))
    ...
    initial_balance = fields.Float(compute="_compute_values", digits=(16, 4))
